...
const Contenido = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "nombre") value = e.target.files[0];
  ...
  const subirContenido = () => {
    ...
    formdata.append("archivo", values.nombre);
    ...
    uploadContent(formdata).then((data) => {
      ...
      else setMessageNotification("No se pudo subir el documento");
    });
  };
  ...
  const handleClickSelected = (content) => {
    setContentSelected(content);
    setComments(content.comentarios);
    setShowModal(true);
  };
  ...
  let contentsCurrent = contentsForTag.length === 0 ? contents : contentsForTag;
        ...
        <section className="container-contenido">
          {isLoading ? (
            ...
            <div className="contenido-content">
              {contentsCurrent.length === 0 ? (
                <NotAvalaible>No hay contenidos disponibles</NotAvalaible>
              ) : (
                <div className="row-content-user">
                  {contentsCurrent.map((content, index) => {
                    return (
                      <div key={index + 1}>
                        <div className="col-content-user">
                          <div className="card-content-user">
                            <div className="card-body-content-user">
